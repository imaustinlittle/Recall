...
function DashboardContent() {
  ...
  // Date comes from URL — kept in sync by the sidebar calendar
  const selectedDate = searchParams.get("date");

  const now = new Date();
  // Derive which month to display from selected date (or default to current month)
  const viewDate = selectedDate ? new Date(selectedDate + "T00:00:00") : now;
  ...
  // Meetings for the visible month
  useEffect(() => {
    if (!user) return;
  ...
  // Filter to selected date client-side
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0 flex items-center gap-3">
            <span className={[
              ...
              meeting.status === "failed" ? "bg-red-400" : "bg-gray-300",
            ].join(" ")} />
    ...
    <div className="max-w-3xl text-center py-20 bg-white border border-dashed border-gray-200 rounded-xl">
      {hasFilter ? (
        ...
        <>
          ...
          <button onClick={onNew} className="inline-flex items-center gap-1.5 text-brand-600 hover:text-brand-700 text-sm font-medium">
            <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" />
            </svg>
            New meeting
